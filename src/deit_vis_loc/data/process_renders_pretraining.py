...
if '__main__' == __name__:
    ...
    data_dir   = os.path.join(args['sparse_dir'], 'sparse_queries', f'query_{modality}')
    info_fpath = os.path.join(data_dir, 'datasetInfoClean.csv')
    meta_it    = util.take(args['n_images'], commons.iter_csv_file(parse_line, info_fpath))

    rd_dir       = os.path.join(args['output_dir'], 'renders', 'pretraining', modality)
    rd_im_dir    = os.path.join(rd_dir, str(resolution))
    prog_printer = ft.partial(print_progress, sum(
        map(util.first, zip(it.repeat(1), util.take(args['n_images'], open(info_fpath))))))

    os.makedirs(rd_im_dir, exist_ok=True)
    with open(os.path.join(rd_dir, 'meta.bin'), 'wb') as meta_f:
        prog_printer((0, None))
        util.dorun(
            map(prog_printer, enumerate(
                map(ft.partial(save_processed, rd_im_dir, meta_f),
                    map(ft.partial(process_render, data_dir, resolution), meta_it)), 1)))
