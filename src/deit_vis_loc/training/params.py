...
def parse(params):
    is_nonempty     = lambda x: (x is None) or x.strip()
    ...
    checker         = util.make_checker({
        ...
        'deit_model'        : util.make_validator('deit_model must be a non-empty string', is_nonempty),
    ...
    if checker(params):
        raise ValueError('Invalid train params ({})'.format(', '.join(checker(params))))
    return params
