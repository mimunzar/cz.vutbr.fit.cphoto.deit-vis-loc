...
IS_NON_NEG  = lambda n: IS_NUM(n) and 0 <= n
IS_POS      = lambda n: IS_NUM(n) and 0 < n
IS_POS_INT  = lambda n: IS_INT(n) and 0 < n
IS_MISSING  = lambda p, k: k not in p

POS_VALIDATORS = {
    'samples'     : util.make_validator('samples must be positive int', IS_POS_INT),
    'dist_m'      : util.make_validator('pos_dist_m must be positive int', IS_NON_NEG),
    'dist_tol_m'  : util.make_validator('dist_tolerance_m must be positive int', IS_NON_NEG),
    'yaw_deg'     : util.make_validator('yaw_deg must be positive int', IS_NON_NEG),
    'yaw_tol_deg' : util.make_validator('yaw_tolerance_deg must be positive int', IS_NON_NEG),
}
