...
def test_cosine_dist():
    o = torch.ones ((1, 2))
    z = torch.zeros((1, 2))
    assert crosslocate.cosine_dist(o, o) == 0
    assert crosslocate.cosine_dist(z, z) == 1
    assert crosslocate.cosine_dist(o, z) == 1
    assert crosslocate.cosine_dist(z, o) == 1
...
def test_triplet_loss():
    ...
    loss = ft.partial(crosslocate.triplet_loss, 0.)
    ...
    loss = ft.partial(crosslocate.triplet_loss, .5)
